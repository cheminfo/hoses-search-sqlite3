...
export function getSpheresStats(stats) {
  ...
  for (const key in stats) {
    ...
    for (const stat of Object.values(stats[key])) {
      ...
      let currentStats = spheres.filter(stats => stats.sphere === stat.sphere && stats.atomLabel === atomLabel)[0];
      if (!currentStats) {
        currentStats = { sphere: stat.sphere, atomLabel: atomLabel, min: stat.boxplot.min, max: stat.boxplot.max, diffs: [] };
    ...
    for (const sphere of spheres) {
      sphere.diffsHistogram = xHistogram(sphere.diffs, { min: -0.1, max: 10.1, nbSlots: 51 });
    }
  }
