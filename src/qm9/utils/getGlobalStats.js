...
export function getGlobalStats(stats) {
  ...
  for (const key in stats) {
    ...
    for (const stat of Object.values(stats[key])) {
      ...
      currentSphereStats.values.push(
        ...stat.values.filter((value) => value && !isNaN(value)),
      );
    }
    for (const currentSphereStats of spheresStats) {
      ...
      if (currentSphereStats.diffs.length > 0) {
        currentSphereStats.diffsHistogram = xHistogram(
          currentSphereStats.diffs,
          { min: -0.1, max: 10.1, nbSlots: 51 },
        );
      }
      if (currentSphereStats.medians.length > 0) {
        currentSphereStats.mediansHistogram = getHistogramMedians(
          currentSphereStats.medians,
          currentSphereStats.min,
          currentSphereStats.max,
        );
      }
    }
