...
const statsKeys = ['gw', 'dksCharged', 'dksNeutral', 'cm5', 'mulliken'];

const stats = {};
console.log('Loading densities');
const densities = await getDensities(
  readFileSync(
    new URL('QM9_mulliken_CM5_electron_density.txt', import.meta.url),
    'utf8',
  ),
  { maxSphereSize: 2 },
);
console.log('Loading entries');
const entriesQM9 = await getEntries(
  readFileSync(new URL('qm9.xyz', import.meta.url), 'utf8'),
  { maxSphereSize: 2, stats, densities, statsKeys },
);
...
writeFileSync(
  new URL('../../src/data/qm9.json', import.meta.url),
  stringify({
    entries: entriesQM9,
    stats: statsAsArrays,
    spheres,
    globalStats,
    statsKeys: statsKeys.map((key) => ({ key })),
  }),
);
...
function appendBoxplot(stats, statsKeys) {
  for (const key of statsKeys) {
    for (const hose in stats[key]) {
      ...
      stats[key][hose].boxplot = xBoxPlot(stats[key][hose].values, {
        allowSmallArray: true,
      });
      stats[key][hose].histogram = xHistogram(
        stats[key][hose].values,
        elementRanges[atomLabel],
      );
    }
  }
}
